...
const interactionsDictionary: Record<string, InteractionResult> = {
...
const TypeIcon = ({ type }: { type: "Pill" | "Leaf" }) =>
  type === "Pill" ? <Pill className="h-5 w-5" /> : <Leaf className="h-5 w-5" />;

export default function LaboratorioInteracoesGame() {
  ...
  const handleMix = () => {
    ...
    if (isNew) {
      ...
      setDiscoveredKeys((prev) => new Set(prev).add(key));
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-muted-foreground text-sm">
            Interações perigosas descobertas: {discoveredDanger}/{totalDangerInteractions}
          </p>
        </div>
        <div className="flex items-center gap-2 bg-yellow-100 text-yellow-800 px-4 py-2 rounded-full font-bold text-lg">
              ...
              <DialogHeader>
                ...
                <DialogTitle
                  className={`text-center text-xl ${
                    lastResult.type === "danger" ? "text-red-700" : "text-green-700"
                  }`}
                >
                  {lastResult.title}
                </DialogTitle>
...
function SlotBox({ item, onRemove }: { item?: InventoryItem; onRemove: () => void }) {
  if (!item) {
    return (
      <div className="w-36 h-28 rounded-xl border-2 border-dashed border-muted-foreground/30 flex items-center justify-center text-muted-foreground text-sm">
        Vazio
      </div>
    );
  }
