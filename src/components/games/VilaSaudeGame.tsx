...
/* ───────── component ───────── */
export default function VilaSaudeGame() {
  ...
  const takeMed = useCallback(
    (id: number) => {
      ...
      if (med) {
        setStats((s) => ({ ...s, coins: s.coins + med.reward }));
        toast({
          title: "Muito bem! 💊",
          description: `Ganhaste ${med.reward} moedas por tomar ${med.name}!`,
        });
      }
    },
    [medications, toast]
  ...
  const upgradeBuilding = useCallback(
    (id: number) => {
        ...
        prev.map((item) =>
          item.id === id
            ? {
                ...
                upgradeCost: isMax
                  ? item.upgradeCost
                  : Math.round(item.upgradeCost * 1.2),
              }
            : item
        )
      );
      ...
      toast({
        title: "Construção melhorada! 🏗️",
        description: `${b.name} subiu para o nível ${newLevel}!`,
      });
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between text-sm font-medium">
          ...
          <span>
            {takenCount}/{medications.length} remédios
          </span>
        </div>
      ...
      <div className="flex gap-4">
        <div className="flex items-center gap-2 rounded-xl bg-yellow-100 px-4 py-2 font-semibold text-yellow-700">
          <Coins className="h-5 w-5" /> {stats.coins}
        </div>
        <div className="flex items-center gap-2 rounded-xl bg-orange-100 px-4 py-2 font-semibold text-orange-600">
          <Flame className="h-5 w-5" /> {stats.streak} dias seguidos
        </div>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        {/* ── left: medication diary ── */}
        <section className="space-y-3">
          <h2 className="flex items-center gap-2 text-lg font-bold">
            <Pill className="h-5 w-5 text-emerald-600" /> Diário do Medicamento
          </h2>
          {medications.map((med) => (
            <Card
              key={med.id}
              className={med.taken ? "border-emerald-300 bg-emerald-50" : ""}
            >
              <CardContent className="flex items-center justify-between py-4">
                ...
                <Button
                  ...
                  className={
                    med.taken
                      ? "gap-1 bg-emerald-200 text-emerald-700"
                      : "gap-1 bg-emerald-600 hover:bg-emerald-700"
                  }
                  onClick={() => takeMed(med.id)}
                >
                  {med.taken ? (
                    <>
                      Tomado <Check className="h-4 w-4" />
                    </>
                  ) : (
                    `Marcar como Tomado (+${med.reward} 🪙)`
                  )}
                </Button>
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-2 gap-3">
            {buildings.map((b) => {
              ...
              return (
                <div
                  key={b.id}
                  className="flex flex-col items-center rounded-2xl bg-white/60 p-4 text-center backdrop-blur-sm transition-transform hover:scale-[1.02]"
                >
                  <Icon
                    className={getIconColor(b.level, b.maxLevel)}
                    size={getIconSize(b.level, b.maxLevel)}
                  />
                  ...
                  {isMax ? (
                    ...
                    <Button
                      ...
                      variant="outline"
                      disabled={stats.coins < b.upgradeCost}
                      onClick={() => upgradeBuilding(b.id)}
                    >
                      Melhorar ({b.upgradeCost} <Coins className="h-3 w-3" />)
                    </Button>
                  )}
                </div>
              );
